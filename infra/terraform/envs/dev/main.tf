...
module "nsg_sonarqube" {
  source = "../../modules/nsg"

  name                = "nsg-sonarqube-${local.name_prefix}"
  resource_group_name = module.resource_group.name
  location            = module.resource_group.location
  ...
  security_rules = [
    {
      name                       = "Allow-Sonar-From-AppGateway"
      priority                   = 100
      direction                  = "Inbound"
      access                     = "Allow"
      protocol                   = "Tcp"
      ...
      description                = "Allow SonarQube web traffic from Application Gateway."
    },
    {
      name                       = "Allow-Sonar-From-Ops"
      priority                   = 110
      direction                  = "Inbound"
      access                     = "Allow"
      protocol                   = "Tcp"
      ...
      destination_address_prefix = "*"
      destination_port_range     = "9000"
      description                = "Allow runner and admins in ops subnet to access SonarQube."
    },
    {
      name                       = "Allow-SSH-From-Ops"
      priority                   = 120
...
module "sonarqube_vm" {
  ...
  name                = "vm-sonar-${local.name_prefix}-01"
  resource_group_name = module.resource_group.name
  location            = module.resource_group.location
  subnet_id           = module.network.subnet_ids["sonarqube"]
...
module "application_gateway" {
  ...
  sonarqube_private_ip = module.sonarqube_vm.private_ip_address
